...
ADD 
  (
    ...
    %s VARCHAR2(6) NOT NULL CHECK(
      %s IN ('insert', 'update', 'delete')
    ), 
...
`
	...
	queryTriggerCreate = `
CREATE 
OR REPLACE TRIGGER %s 
AFTER 
  INSERT 
  OR 
UPDATE 
  OR DELETE ON %s FOR EACH ROW DECLARE transaction_type VARCHAR2(6);
BEGIN transaction_type := CASE WHEN INSERTING THEN 'insert' WHEN UPDATING THEN 'update' WHEN DELETING THEN 'delete' END;
IF INSERTING 
OR UPDATING THEN %s ELSE %s END IF;
END;
`
