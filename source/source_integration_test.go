...
func TestSource_snapshotRead(t *testing.T) {
	...
	SELECT * FROM p`, cfg[config.Table]))
	is.NoErr(err)
...
func TestSource_cdcRead(t *testing.T) {
	...
	_, err = repo.DB.Exec(fmt.Sprintf("UPDATE %s SET age = 33 WHERE ID=2", cfg[config.Table]))
	is.NoErr(err)
	...
	_, err = repo.DB.Exec(fmt.Sprintf("DELETE FROM %s WHERE ID=3", cfg[config.Table]))
	is.NoErr(err)
